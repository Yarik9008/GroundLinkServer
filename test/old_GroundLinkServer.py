...
log_file_path = str(BASE_DIR / "lorett_monitor.log")
...
TG_SESSION = os.getenv("TG_SESSION", str(BASE_DIR / "telegram"))
COMM_DB_PATH = os.getenv("COMM_DB_PATH", str(BASE_DIR / "comm_passes.db"))
ALL_PASSES_DB_PATH = os.getenv("ALL_PASSES_DB_PATH", str(BASE_DIR / "all_passes.db"))
...
def update_all_passes_db_for_date(target_date: str) -> None:
    ...
    base_logs_dir = BASE_DIR / "logs" / year / month / date_str
...
def download_logs_for_date(target_date: str) -> None:
    ...
    year, month, date_str, _ = get_date_paths(target_date)
    logs_dir = BASE_DIR / "logs" / year / month / date_str
...
def analyze_satellite_logs_for_station(
    ...
    while current_date <= end_date:
        ...
        base_logs_dir = BASE_DIR / "logs" / year / month / date_str / station_name
...
def analyze_downloaded_logs(target_date: str) -> None:
    ...
    if stations:
        ...
        graphs_dir = BASE_DIR / "report" / year / month / date_folder
        try:
...
def load_stations_from_config_for_analysis(config_path: Path = BASE_DIR / "config.json") -> Tuple[dict, dict]:
    """
