...
- **Telegram** — синхронизация списка коммерческих пролётов из канала (заказ сессий приёма), с сохранением пометки **тестовых** коммерческих витков (тип/комментарий).
- **Графики** — % пустых пролётов за 7 дней (общий и по станциям), % непринятых коммерческих за 7 дней.
- **Email-сводка** — HTML-письмо со статистикой по станциям, коммерческими пролётами, графиками и списком непринятых коммерческих с ссылками на графики.
- **Еженедельное письмо** — дополнительная email-сводка за последние 7 дней, отправляется **по воскресеньям (UTC)**.
...
- **email** — `enabled`, SMTP-параметры, отправитель, получатели, тема:
  - `subject` — тема ежедневного письма
  - `weekly_subject` — тема еженедельного письма (например: `Сводка работы станций за неделю`)
  - `debug_recipient` — адрес для отладочной отправки
  - `attach_report` — прикреплять файлы/графики (если включено)
...
```bash
python GroundLinkServer.py [start_date] [end_date] [--sch] [--off-email] [--debag-email] [--weekly-email-all|--weekly-email-debug]
```
...
- **--weekly-email-all** — weekly-сводка (воскресенье, UTC): отправить на все адреса (To+Cc).
- **--weekly-email-debug** — weekly-сводка (воскресенье, UTC): отправить только на `debug_recipient`.
...
# По расписанию, weekly-сводка в воскресенье уйдёт только на debug_recipient
python GroundLinkServer.py --sch --weekly-email-debug
```
...
5. Для каждого дня в диапазоне строится сводка по станциям, генерируются графики (общий и по станциям за 7 дней, коммерческие за 7 дней), при включённой отправке — формируется ежедневное письмо со статистикой и списком непринятых коммерческих.
6. По воскресеньям (UTC) отправляется дополнительное weekly-письмо за последние 7 дней.
7. В коммерческой статистике учитывается текущее время UTC: витки с `rx_start_time` в будущем **не учитываются** (появятся в отчётах, когда наступит их время).

## База данных

Файл по умолчанию: `groundlink.db`. В БД хранятся: пролёты (в т.ч. ссылки на логи/графики), дневная статистика по станциям, коммерческие пролёты (станция, спутник, окно приёма, тип/комментарий из Telegram).
Методы `DbManager` позволяют получать число запланированных/принятых коммерческих пролётов на день/период **с отсечкой по текущему времени UTC** (витки из будущего не считаются), а также список непринятых коммерческих с URL графиков и типом витка (коммерческий/тестовый) для отображения в письме.
